...
const LeaderBoard = () => {
  ...
  const filteredStudents = students
    ...
    .sort((a, b) => (filter === 'points' ? b.points - a.points : a.username.localeCompare(b.username)));
            ...
            >
              <div className="flex justify-between items-center">
                <div className="flex items-center gap-4">
                  <span className={`text-xl font-bold ${index === 0 ? 'text-yellow-500' : index === 1 ? 'text-gray-500' : 'text-orange-400'}`}>{index + 1}</span>
                  <span className="text-lg font-semibold">{student.username}</span>
                  <div>{student.badges.map((badge, i) => <span key={i} className="ml-1">{badge}</span>)}</div>
                </div>
